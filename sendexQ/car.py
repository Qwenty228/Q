            ...
            for x2 in range(-N_SIZE, N_SIZE):
                for y2 in range(0, N_SIZE + 2):
                    for length in range(5,16):
                        q_table[((x1, y1),(x2, y2),length)] = [np.random.uniform(-5, 0) for i in range(3)]
...
for episode in range(HM_EPISODES):
    ...
    while not done: # main loop
        ...
        obs = (player - nearest, player - sec_near, nearest.rect.w//10)
        ...
        new_obs = (player - nearest, player - sec_near, nearest.rect.w//10)
